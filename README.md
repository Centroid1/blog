...
* 게시글 수정과 삭제 권한이 있으면 '수정하기', '삭제하기' 두 버튼이 화면에 보이게 하고 권한이 없으면 버튼을 화면에 출력하지 않는다.
  * 게시글의 수정과 삭제 권한은 현재 로그인한 사용자의 Grade가 MANAGER이거나 로그인한 사용자가 게시글을 작성한 사용자와 같은 사용자이면 권한이 존재한다.
* post데이터의 댓글 List를 출력한다. 각 댓글에 대하여 username과 댓글의 내용이 화면에 출력된다.
  ...
  * 댓글의 삭제 권한은 현재 로그인한 사용자의 Grade가 MANAGER이거나 로그인한 사용자가 댓글을 작성한 사용자와 같은 사용자이면 권한이 존재한다.
