...
public class MainController {
	...
	/*
	 * 프로그램 내에서 구현한 사항은 아니지만
	 * 이 프로젝트를 진행하면서 작성되는 post(게시글)의 개수가 100만개 이하라고 가정한다.
	 * 
	 * 
	 */
	...
	public String writePost(@SessionAttribute(name = "loginInfo", required = false) Long memberId, 
		...
		if(postCreate.getTitle().isEmpty()) result.addError(new FieldError("postCreate", "title", "제목 입력"));
		if(postCreate.getContent().isEmpty()) result.addError(new FieldError("postCreate", "content", "내용 입력"));
		if(result.hasErrors()) {
			model.addAttribute("username", member.getUsername());
			return "postWrite";
		}
	...
	public String editPost(@PathVariable(name = "id") Long postId, @SessionAttribute(name = "loginInfo", required = false) Long memberId, 
		...
		if(postEdit.getTitle().isEmpty()) result.addError(new FieldError("postEdit", "title", "제목 입력"));
		if(postEdit.getContent().isEmpty()) result.addError(new FieldError("postEdit", "content", "내용 입력"));
		if(result.hasErrors()) {
			Member member = memberService.findById(memberId);
			model.addAttribute("username", member.getUsername());
			return "postEdit";
